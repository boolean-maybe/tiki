...
// DocumentReadStore is the Phase 4 document-first read surface. Callers that
// need the document.Document shape (e.g. the wiki/detail view layer) use this
// interface. Both DocumentReadStore and ReadStore project the same in-memory
// state; DocumentReadStore uses the document.Document type while ReadStore and
// TikiReadStore project to task.Task or tiki.Tiki respectively.
type DocumentReadStore interface {
	...
	// GetAllDocuments returns every loaded document, plain and workflow alike.
