...
// the welcome-tiki writer must quote it in the YAML frontmatter so the next
// strict load parses it as a string, not an integer that drops leading
// zeros.
//
// Without the fix, yaml.v3 decodes `id: 000001` as int 1, FrontmatterID
// stringifies as "1", and ValidateID rejects it — the file is effectively
// unloadable by the very store that wrote it.
func TestBootstrap_AllDigitIDSurvivesLoad(t *testing.T) {
	repoDir := setupInitTest(t)

	// Force the generator to emit an all-digit id for the welcome tiki and
	...
	config.GenerateRandomIDForTest = func() string {
		...
		// fall back to fresh ids for subsequent calls.
	...
	// The welcome tiki lands directly under .doc/<ID>.md.
	docDir := filepath.Join(repoDir, ".doc")
	target := filepath.Join(docDir, "000001.md")
	content, err := os.ReadFile(target)
	if err != nil {
		t.Fatalf("welcome tiki for all-digit id not written at expected path: %v", err)
	}
