...
// so the base evalHas ran with ctx.current pointing at a sentinel tiki
...
func TestPhase5_Trigger_HasNewFieldTrueWhenPresent(t *testing.T) {
	...
	// deny would fire) only when the new tiki has no assignee field.
	...
	tc := &TriggerContext{Old: nil, New: tikiFromFixture(withAssignee)}
...
func TestPhase5_Trigger_HasNewFieldFalseWhenAbsent(t *testing.T) {
	...
	if !ok {
		t.Fatal("guard `not has(new.assignee)` should match when assignee is absent on the new tiki")
	}
}
...
func TestPhase5_Trigger_HasOldFieldTrueWhenPresent(t *testing.T) {
	...
	newTiki := &tikiFixture{ID: "TK01", Status: "done"}
	tc := &TriggerContext{Old: tikiFromFixture(oldTiki), New: tikiFromFixture(newTiki)}
	...
	if !ok {
		t.Fatal("guard `has(old.status)` should match when the old tiki carried a status")
	}
}
...
func TestPhase5_Trigger_HasOldFieldFalseWhenAbsent(t *testing.T) {
	...
	oldTiki := &tikiFixture{ID: "TK01"} // no assignee declared
	newTiki := &tikiFixture{ID: "TK01", Assignee: "alice"}
	tc := &TriggerContext{Old: tikiFromFixture(oldTiki), New: tikiFromFixture(newTiki)}
	...
	if ok {
		t.Fatal("guard `has(old.assignee)` should NOT match when the old tiki had no assignee")
	}
}
...
func TestPhase5_Trigger_HasOldAndHasNewResolveDistinctTikis(t *testing.T) {
	te := newTestTriggerExecutor()
	p := newTestParser()
	...
	oldTiki := &tikiFixture{ID: "TK01"} // no status key
	newTiki := &tikiFixture{
		ID: "TK01", Status: "ready",
		WorkflowFrontmatter: map[string]interface{}{"status": "ready"},
	}
	...
	oldTiki2 := &tikiFixture{
		ID: "TK02", Status: "ready",
		WorkflowFrontmatter: map[string]interface{}{"status": "ready"},
	}
	newTiki2 := &tikiFixture{
		ID: "TK02", Status: "done",
		WorkflowFrontmatter: map[string]interface{}{"status": "done"},
	}
	tc2 := &TriggerContext{Old: tikiFromFixture(oldTiki2), New: tikiFromFixture(newTiki2)}
