...
func TestExecuteCreateEmptyTasks(t *testing.T) {
	...
	result, err := e.testExec(stmt, []*tikiFixture{})
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
...
func TestSetFieldTypeMismatches(t *testing.T) {
	e := newTestExecutor()
	tk := tikiFromFixture(&tikiFixture{ID: "T1", Title: "x", Status: "ready", Type: "bug"})
...
func TestSetFieldDescriptionToEmpty(t *testing.T) {
	e := newTestExecutor()
	tk := tikiFromFixture(&tikiFixture{ID: "T1", Title: "x", Status: "ready", Description: "some desc"})
...
func TestSetFieldPointsToEmpty(t *testing.T) {
	e := newTestExecutor()
	tk := tikiFromFixture(&tikiFixture{ID: "T1", Title: "x", Status: "ready", Points: 5})
...
func TestSetFieldDescriptionNonString(t *testing.T) {
	e := newTestExecutor()
	tk := tikiFromFixture(&tikiFixture{ID: "T1", Title: "x", Status: "ready", Description: "old"})
...
func TestExecutor_CustomFieldNilDelete(t *testing.T) {
	e := newCustomExecutor()
	tk := tikiFromFixture(&tikiFixture{
		ID: "T1", Title: "x", Status: "ready",
		CustomFields: map[string]interface{}{
			"notes": "hello",
			"score": 42,
